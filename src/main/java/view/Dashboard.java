...
public class Dashboard extends JFrame {
    ...
    public Dashboard(User currentUser) {
        ...
        if (currentUser.getRole().equalsIgnoreCase("ADMIN")) {
            ...
            // Dentro del bloque: if (currentUser.getRole().equalsIgnoreCase("ADMIN"))
            JMenuItem parkingConfigItem = new JMenuItem("Configurar Parqueo");
           parkingConfigItem.addActionListener(e -> {
            // Cerramos el dashboard para evitar múltiples ventanas abiertas
            // Pasamos el currentUser para no perder la sesión del administrador
            new ParkingManagementFrame(currentUser).setVisible(true);
            this.dispose();
        });
            menuAdmin.add(parkingConfigItem);
